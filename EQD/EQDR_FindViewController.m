...
@interface EQDR_FindViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    UITableView *tableV;
...
-(void)loadRequestData{
    ...
    [WebRequest Articles_Get_Article_ByTimeWithpage:@"0" time:time And:^(NSDictionary *dic) {
...
-(void)loadOtherData
{
    [WebRequest Articles_Get_Article_ByTimeWithpage:page time:time And:^(NSDictionary *dic) {
        [tableV.mj_header endRefreshing];
        [tableV.mj_footer endRefreshing];
        if ([dic[Y_STATUS] integerValue]==200) {
            NSDictionary  *tdic = dic[Y_ITEMS];
...
- (void)viewDidLoad {
    ...
    tableV = [[UITableView alloc]initWithFrame:CGRectMake(0, DEVICE_TABBAR_Height, DEVICE_WIDTH,CGRectGetMinY(self.tabBarController.tabBar.frame)-DEVICE_TABBAR_Height) style:UITableViewStylePlain];
