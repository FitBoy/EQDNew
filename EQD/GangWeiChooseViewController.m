...
#import "ZuZhiModel.h"
@interface GangWeiChooseViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    UITableView *tableV;
...
@implementation GangWeiChooseViewController
...
-(void)loadRequestData{
    ...
    hud.label.text = @"正在加载";
 /*   [WebRequest Com_SelectSection_PartWithpparent:selected_model.departId And:^(NSArray *arr){
...
-(void)loadGangwei
{
    ...
    hud.mode = MBProgressHUDModeAnnularDeterminate;
    hud.label.text = @"正在加载";
 /*   [WebRequest Com_SelectCareer_PartWithpid:selected_model.departId And:^(NSDictionary *dic) {
        [hud hideAnimated:NO];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        GangweiModel *model =arr_gangwei[indexPath.row];
        if ([self.delegate respondsToSelector:@selector(gangweiModel:indexPath:)]) {
            [self.delegate gangweiModel:model indexPath:indexPath];
