...
@interface Accident_AddViewController ()<UITableViewDataSource,UITableViewDelegate,FBTextVViewControllerDelegate,FBTextvImgViewControllerDelegate,FBTextFieldViewControllerDelegate>
{
    UITableView *tableV;
    NSArray *arr_names;
...
@implementation Accident_AddViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    user = [WebRequest GetUserInfo];
    self.navigationItem.title =@"添加事故记录";
    arr_names = @[@"车号",@"责任人",@"事故发生时间",@"事故地点",@"责任比例",@"费用(元)",@"备注"];
    arr_contents = [NSMutableArray arrayWithArray:@[self.plateNumber,@"请输入",@"请选择",@"请输入",@"请输入",@"请输入",@"请输入",]];
...
-(void)tijiaoClick
{
    ...
    if (temp==0) {
        ...
        hud.label.text = @"正在提交";
        [WebRequest Com_Vehicle_Add_vehicleAccidentWithuserGuid:user.Guid comid:user.companyId createrName:user.username vehicleId:self.Id plateNumber:self.plateNumber personLiableName:arr_contents[1] theTime:arr_contents[2] thePlace:arr_contents[3] dutyRatio:arr_contents[4] cost:arr_contents[5] remark:arr_contents[6] arr_imgs:arr_imgs And:^(NSDictionary *dic) {
