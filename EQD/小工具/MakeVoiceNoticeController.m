...
- (void)viewDidLoad {
   ...
   right = [[UIBarButtonItem alloc]initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(commitVoiceNotice)];
...
- (void)initEditView{
    ...
    [deleteBtn addTarget:self action:@selector(deleteCurrentRecord) forControlEvents:UIControlEventTouchUpInside];
...
- (void)commitVoiceNotice
{
    ...
    if([self.delegate respondsToSelector:@selector(getvoiceData:time:)])
    {
        long long fileSize = [self fileSizeAtPath:self.mp3PathStr]/1024.0;
        NSString *fileSizeStr = [NSString stringWithFormat:@"%lld",fileSize];
        
        NSString *tstr = [NSString stringWithFormat:@"%ld \" %@kb  ",(long)self.countNum,fileSizeStr];
        [self.delegate getvoiceData:data time:tstr];
    }
...
//    NSLog(@"mp3PathStr:%@",self.mp3PathStr);
    NSLog(@"countNum %ld , fileSizeStr : %@",self.countNum,fileSizeStr);
    /*
    if(self.countNum >60)
    {
        MBProgressHUD *hud=[MBProgressHUD showHUDAddedTo:self.view  animated:YES];
        hud.mode = MBProgressHUDModeText;
        hud.label.text =@"支持最大录音长度45秒";
...
{
    if (!_editView) {
        _editView = [[UIView alloc]initWithFrame:CGRectMake(0,DEVICE_TABBAR_Height, kSCREEN_WIDTH, 200)];
